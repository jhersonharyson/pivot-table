...
(function() {
  ...
  aggregators = (function(tpl) {
    return {
      ...
      "Acima de 80%": tpl.sumOverSumBound80(true, usFmt),
      "Abaixo de 80%": tpl.sumOverSumBound80(false, usFmt),
      "Soma das frações do Total": tpl.fractionOf(tpl.sum(), "total", usFmtPct),
      "Soma das frações das Linhas": tpl.fractionOf(tpl.sum(), "row", usFmtPct),
      "Soma das frações das colunas ": tpl.fractionOf(tpl.sum(), "col", usFmtPct),
      "Contagem das frações do Total": tpl.fractionOf(tpl.count(), "total", usFmtPct),
      "Contagem das frações das Linhas": tpl.fractionOf(tpl.count(), "row", usFmtPct),
      "Contagem das frações das Colunas": tpl.fractionOf(tpl.count(), "col", usFmtPct)
    };
  })(aggregatorTemplates);

  renderers = {
    "Tabela": function(pvtData, opts) {
      return pivotTableRenderer(pvtData, opts);
    },
    "Tabela Barchart": function(pvtData, opts) {
      return $(pivotTableRenderer(pvtData, opts)).barchart();
    },
    "Heatmap": function(pvtData, opts) {
      return $(pivotTableRenderer(pvtData, opts)).heatmap();
    },
    "Linha Heatmap": function(pvtData, opts) {
      return $(pivotTableRenderer(pvtData, opts)).heatmap("rowheatmap");
    },
    "Colunas Heatmap": function(pvtData, opts) {
  ...
  pivotTableRenderer = function(pivotData, opts) {
    ...
    for (j in colAttrs) {
      ...
      if (parseInt(j) === 0) {
        ...
        $(".pvtTotalLabel").text(opts.localeStrings.totals);
    ...
    if (rowAttrs.length !== 0) {
      ...
      if (colAttrs.length === 0) {
        th.className = "pvtTotalLabel";
        th.value = opts.localeStrings.totals;
      }
      tr.appendChild(th);
      result.appendChild(tr);
    }
    for (i in rowKeys) {
      ...
      for (j in colKeys) {
        ...
        td.value = aggregator.format(val);
      ...
      td.value = totalAggregator.format(val);
      td.setAttribute("data-value", val);
